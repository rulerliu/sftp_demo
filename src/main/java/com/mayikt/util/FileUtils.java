...
public class FileUtils {
	...
	private static final String RESPCODE_ERROR = "操作失败!";
	...
	public static String getFileNameWithoutSuffix(String fileName) {
		...
		if (file.isDirectory()) {
			log.error("传入地址为文件夹");
			throw new ResourceException(RESPCODE_ERROR, "传入地址为文件夹");
		}
	...
	public static File createFolder(String destDirName) {
		...
		try {
			...
			if (file.mkdirs()) {
				...
				throw new ResourceException(RESPCODE_ERROR, "创建目录【" + destDirName + "】失败");
			}
		}
		catch (Exception e) {
			log.error("创建目录【" + destDirName + "】失败:" + e.getMessage());
			throw new ResourceException(RESPCODE_ERROR, "创建目录【" + destDirName + "】失败:" + e.getMessage());
		}
	...
	public static String readFile(String filePath, String charset) {
		...
		try {
			...
			if (file.isFile() && file.exists()) { // 判断文件是否存在
				...
				log.error("找不到指定文件");
				throw new ResourceException(RESPCODE_ERROR, "找不到指定文件");
			}
		}
		catch (Exception e) {
			log.error("读取文件内容失败：" + e.getMessage());
			throw new ResourceException(RESPCODE_ERROR, "读取文件内容失败：" + e.getMessage());
		}
		return content;
	}
	...
	private static String loadOthersFile(String file, String encoding) {
		...
		try {
			...
			throw new ResourceException(RESPCODE_ERROR, "读取文件内容出错:" + e.getMessage());
		}
		finally {
			try {
				if (null != read) {
	...
	private static String loadUTF8File(String file) {
		...
		try {
			...
			throw new ResourceException(RESPCODE_ERROR, "读取UTF-8文件错误：" + ex.getMessage());
		}
		finally {
			try {
				if (null != writer) {
	...
	public static void copyFile(String sourcePath, String targetPath) {
		...
		if (!sourceFile.exists()) {
			throw new ResourceException(RESPCODE_ERROR, "源文件不存在");
		}
		if (!sourceFile.isFile()) {
			throw new ResourceException(RESPCODE_ERROR, "源路径不是一个文件");
		}
		...
		try {
			...
			throw new ResourceException(RESPCODE_ERROR, "复制文件失败:" + e.getMessage());
		}
	}
	...
	public File createTempFile(String filePath, String fileName, byte[] mdf) {
		try {
			...
			throw new ResourceException(RESPCODE_ERROR, "创建临时文件失败：" + e.getMessage());
		}
		return file;
	}
	...
	public static void DeleteFolder(String deletePath) {
		try {
			...
			throw new ResourceException(RESPCODE_ERROR, "删除失败:" + e.getMessage());
		}
	...
	private static void deleteFile(String filePath) {
		...
		try {
			...
			if (file.isFile() && file.exists()) {
				...
				throw new ResourceException(RESPCODE_ERROR, "文件不存在或路径错误");
			}
		}
		catch (Exception e) {
			log.error(e.getMessage());
			throw new ResourceException(RESPCODE_ERROR, e.getMessage());
		}
	}
	...
	private static void deleteDirectory(String dirPath) {
		...
		try {
			...
			if (!file.exists() || !file.isDirectory()) {
				...
				throw new ResourceException(RESPCODE_ERROR, "目录不存在或不是目录");
			}
			...
			if (file.delete()) {
				...
				throw new ResourceException(RESPCODE_ERROR, "删除当前文件夹失败");
			}
		}
		catch (Exception e) {
			log.error(e.getMessage());
